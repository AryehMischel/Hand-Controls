/* modified on 9/6/2024 by @https://github.com/AryehMischel, so the position and rotation of the hand models can be adjusted on the fly.
/* global THREE */
...
module.exports.Component = registerComponent('hand-controls', {
  ...
  init: function () {
    var self = this;
    var el = this.el;
    el.offSetPosition = this.data.offSetPosition;
    el.offSetRotation = this.data.offSetRotation; 
  ...
  update: function (previousHand) {
    ...
    if (hand !== previousHand) {
      ...
      this.loader.load(handmodelUrl, function (gltf) {
        ...
        var handModelOrientationZ = hand === 'left' ? degreesToRadians(el.offSetRotation.z): -(degreesToRadians(el.offSetRotation.z));
        var handModelOrientationX = el.sceneEl.hasWebXR ? -degreesToRadians(el.offSetRotation.x): 0;  
        ...
        mesh.position.set(el.offSetPosition.x, el.offSetPosition.y, el.offSetPosition.z);
        mesh.rotation.set(handModelOrientationX, degreesToRadians(el.offSetRotation.y), handModelOrientationZ);
